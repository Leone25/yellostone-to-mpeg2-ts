import util_1 from "yellowstone/dist/util.js";
import transform from "sdp-transform";
import { EventEmitter } from "events";
export default class AACTransport extends EventEmitter {
	...
	processRTPPacket(packet) {
		...
		for (let x = 0; x < audio_data.length; x++) {
			...
			// emit
			this.emit("data", Buffer.concat([header, data]), packet.timestamp);
		}
	}
}
