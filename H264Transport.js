...
export default class H264Transport extends EventEmitter {
	constructor(client, details) {
		...
		client.on("data", (channel, data, packet) => {
			if (channel == details.rtpChannel) {
				if (this.lastTimestamp == null) this.lastTimestamp = packet.timestamp;
	...
	processConnectionDetails(details) {
		...
		this.sps = Buffer.from(sps_base64, "base64");
		this.pps = Buffer.from(pps_base64, "base64");
	}
	...
	processRTPFrame(rtpPackets) {
		...
		for (let x = 0; x < nals.length; x++) {
			this.emit("data", Buffer.concat([H264_HEADER, nals[x]]), this.lastTimestamp);
		}
